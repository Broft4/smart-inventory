...
class User(Base):
    ...
    manager_parent_user_id: Mapped[int | None] = mapped_column(ForeignKey('users.id', ondelete='SET NULL'), nullable=True, index=True)
    referral_commission_percent: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class SelectionTarget(Base):
    ...
    subcategory_id: Mapped[str | None] = mapped_column(Text, nullable=True, index=True)
...
class SalesMotivationDailySnapshot(Base):
    __tablename__ = 'sales_motivation_daily_snapshots'
    __table_args__ = (
        UniqueConstraint('location_point_id', 'snapshot_date', 'model_id', 'item_id', 'fiscalization_status', name='uq_sales_motivation_daily_snapshot_item'),
    )
    ...
    last_sale_date: Mapped[date | None] = mapped_column(Date, nullable=True, index=True)
    days_without_sales: Mapped[int | None] = mapped_column(Integer, nullable=True)
    sold_quantity: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    sold_sales_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    bonus_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    fiscalized_quantity: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    fiscalized_sales_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    non_fiscalized_quantity: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    non_fiscalized_sales_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    unknown_fiscalization_quantity: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    unknown_fiscalization_sales_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
