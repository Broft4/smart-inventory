...
This version does NOT fail because of unrelated foreign key issues elsewhere in
the database. It only rebuilds monthly_expense_entries safely.

Usage:
    python scripts/migrate_monthly_expense_entries_template_nullable_v2.py
    python scripts/migrate_monthly_expense_entries_template_nullable_v2.py --db inventory.db
    python scripts/migrate_monthly_expense_entries_template_nullable_v2.py --no-backup
"""
...
def make_backup(db_path: Path) -> Path:
    ...
    backup_path = db_path.with_name(
        f"{db_path.stem}_backup_before_template_nullable_{timestamp}{db_path.suffix}"
    )
